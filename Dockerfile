...
ENV APACHE_RUN_USER=www-data APACHE_RUN_GROUP=www-data \
    ...
    OCS_DB_SERVER=192.168.31.191 OCS_DB_PORT=3306 OCS_DB_USER=ocs OCS_DB_PASS=ocsweb OCS_DB_NAME=ocsweb \
...
RUN cpan XML::Entities

# Modify Apache configuration to increase post_max_size and upload_max_filesize
RUN sed -i 's/^\(post_max_size = \).*/\11024M/' /etc/php/8.1/apache2/php.ini \
    && sed -i 's/^\(upload_max_filesize = \).*/\11024M/' /etc/php/8.1/apache2/php.ini

# Download and extract OCS Inventory Server
